...
| Table layout acceptance gate (Stage 0 + Stage A) | _pending_ | Stage 0 conservative table mode flag (`KPFU_RENDERED_TABLE_CONTINUATION`, default off) + Stage A rendered acceptance gate (`evaluate_table_layout_acceptance`) surfacing fail/needs_human_review blockers. | No Stage B–E table repairs; no auto-delete of source-bad rows; no TOC/refs/formula/runtime changes. | Bondarev NO-GO (2.1.4/2.3.1/appendix Б + cross-page 1.2.1); Demo NO-GO (1.1.3 cross-page); Rybakov GO; conservative mode inserts no rendered markers; `test_rendered_table_acceptance_gate` + existing table suites pass; `test_phase3` 456/0. | Bondarev, Demo, Rybakov. |
