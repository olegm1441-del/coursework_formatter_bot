...
def _collect_body_entries(document: Document, body_start: int) -> list[TocEntry]:
    ...
    for paragraph in document.paragraphs[body_start:]:
        ...
        if low in _STRUCTURAL_HEADINGS:
            ...
            entries.append(TocEntry(canonical))
...
def _insert_contents_block(document: Document, entries: list[TocEntry], pages: dict[str, int | str]) -> None:
    body_start = find_body_start_index(document)
    ...
    for entry in entries:
        page = str(pages.get(entry.title, _PAGE_PLACEHOLDER))
        p = document.add_paragraph()
        _format_toc_entry(p, entry.title, page)
...
def _find_rendered_page(title: str, lines, *, min_page: int) -> int | None:
    ...
    if len(head) >= 12:
        for line in lines:
            if line.page_num < min_page:
                continue
            if _line_norm(line.text).startswith(head):
                return line.page_num
    return None
...
def _resolve_display_pages(entries: list[TocEntry], lines) -> dict[str, int]:
    ...
    rendered_intro_page = min(intro_pages)
    offset = 3 - rendered_intro_page
    resolved: dict[str, int] = {}
    for entry in entries:
        page = _find_rendered_page(entry.title, lines, min_page=rendered_intro_page)
        if page is None:
            raise ValueError(f"rendered page not found for TOC entry: {entry.title}")
        resolved[entry.title] = page + offset
    return resolved
...
def _replace_contents_entry_pages(document: Document, entries: list[TocEntry], pages: dict[str, int]) -> None:
    ...
    for paragraph, entry in zip(entry_paragraphs, entries):
        _format_toc_entry(paragraph, entry.title, str(pages[entry.title]))
...
def rebuild_static_contents_page(docx_path: str | Path) -> bool:
    ...
    try:
        ...
        _insert_contents_block(document, entries, {entry.title: _PAGE_PLACEHOLDER for entry in entries})
