...
def build_guide_selected_text(guide_code: str) -> str:
    ...
    return (
        "Методичка выбрана.\n\n"
        f"Активная методичка:\n{guide['title']}\n\n"
        "Теперь можно отправить .docx-файл на обработку."
    )
...
def build_start_text(balance: int, is_new: bool, active_guide_title: str) -> str:
    if balance > 0:
        if is_new:
            return (
                ...
                "Продолжая использование бота, вы соглашаетесь на обработку персональных данных в соответствии с Политикой обработки данных: https://docs.google.com/document/d/14Sk5N1ow-x30Dh2dLqYQtUU5-LbdUlemkTleL-THJDc/edit?usp=sharing. 
                
                Отправляя документ, вы подтверждаете,что являетесь его автором и имеете право передавать его для обработки."
            )
        return (
            "С возвращением!\n"
            f"Сейчас доступно оформлений: {balance}\n"
    ...
    return (
        ...
        "Также можно получить бонусы по реферальной ссылке."
    )
